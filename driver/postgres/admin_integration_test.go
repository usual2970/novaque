...
// topic/channel ids and never asserts global table contents.
//
// Ported scenario-for-scenario from driver/mysql/admin_integration_test.go;
// the shared countRow helper lives in postgres_integration_test.go.

// dbDay returns a UTC-midnight time for a SQL date expression (e.g.
// "(NOW() AT TIME ZONE 'UTC')::date - make_interval(days => 2)"), read from
// the DB clock so expectations never drift from the server's bucketing.
func dbDay(t *testing.T, ctx context.Context, db *sql.DB, expr string) time.Time {
	t.Helper()
	var s string
	if err := db.QueryRowContext(ctx, "SELECT TO_CHAR("+expr+`, 'YYYY-MM-DD')`).Scan(&s); err != nil {
...
// killTopicRaw deletes a topic and its children with raw SQL in the KTD7
// child-first order, bypassing the driver entirely (used to stage FK-violation
// and idempotency scenarios).
func killTopicRaw(t *testing.T, ctx context.Context, db *sql.DB, topicID int64) {
	t.Helper()
	for _, q := range []string{
		`DELETE FROM novaque_messages WHERE topic_id = $1`,
		`DELETE FROM novaque_channels WHERE topic_id = $1`,
		`DELETE FROM novaque_stats_daily WHERE topic_id = $1`,
		`DELETE FROM novaque_topics WHERE id = $1`,
	} {
...
// The shared database holds other tests' rows, so assertions filter to this
// test's uniquely prefixed names and check relative order.
...
func TestAdminDailyCounters(t *testing.T) {
	...
	seed := func(dayExpr string, channelID, publish, claim, ack, requeue, dead, purged int64) {
		...
		if _, err := db.ExecContext(ctx, `
			...
			VALUES (`+dayExpr+`, $1, $2, $3, $4, $5, $6, $7, $8)`,
			topicID, channelID, publish, claim, ack, requeue, dead, purged); err != nil {
			t.Fatalf("seed %s ch=%d: %v", dayExpr, channelID, err)
		}
	}
	seed("(NOW() AT TIME ZONE 'UTC')::date", chA, 2, 1, 0, 0, 0, 0)                             // today, channel A
	seed("(NOW() AT TIME ZONE 'UTC')::date", 0, 5, 0, 0, 0, 0, 0)                               // today, sentinel
	seed("(NOW() AT TIME ZONE 'UTC')::date - make_interval(days => 2)", chA, 0, 0, 4, 0, 0, 0)  // D-2, channel A
	seed("(NOW() AT TIME ZONE 'UTC')::date - make_interval(days => 2)", chB, 0, 0, 0, 0, 1, 0)  // D-2, channel B
	seed("(NOW() AT TIME ZONE 'UTC')::date - make_interval(days => 10)", chA, 0, 9, 0, 0, 0, 0) // outside every window
	...
	day2 := dbDay(t, ctx, db, "(NOW() AT TIME ZONE 'UTC')::date - make_interval(days => 2)")
	day0 := dbDay(t, ctx, db, "(NOW() AT TIME ZONE 'UTC')::date")
...
func TestAdminDeleteTopicCascade(t *testing.T) {
	...
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_messages WHERE topic_id = $1`, topicID); n != 3 {
		t.Fatalf("messages before delete = %d, want 3", n)
	}
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_deliveries WHERE channel_id IN ($1, $2)`, aID, bID); n != 4 {
		t.Fatalf("deliveries before delete = %d, want 4", n)
	}
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_stats_daily WHERE topic_id = $1`, topicID); n != 3 {
	...
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_topics WHERE id = $1`, topicID); n != 0 {
		t.Fatalf("topics after delete = %d, want 0", n)
	}
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_channels WHERE topic_id = $1`, topicID); n != 0 {
		t.Fatalf("channels after delete = %d, want 0", n)
	}
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_messages WHERE topic_id = $1`, topicID); n != 0 {
		t.Fatalf("messages after delete = %d, want 0", n)
	}
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_deliveries WHERE channel_id IN ($1, $2)`, aID, bID); n != 0 {
		t.Fatalf("deliveries after delete = %d, want 0 (cascaded)", n)
	}
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_stats_daily WHERE topic_id = $1`, topicID); n != 0 {
...
func TestAdminDeleteChannelCascade(t *testing.T) {
	...
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_deliveries WHERE channel_id = $1`, aID); n != 0 {
		t.Fatalf("deleted channel deliveries = %d, want 0", n)
	}
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_deliveries WHERE channel_id = $1 AND status = 'in_flight'`, bID); n != 1 {
		t.Fatalf("sibling in_flight delivery = %d, want 1 (untouched)", n)
	}
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_messages WHERE id = $1`, msgID); n != 1 {
		t.Fatalf("shared message row = %d, want 1 (kept for the sibling)", n)
	}
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_channels WHERE topic_id = $1`, topicID); n != 1 {
		t.Fatalf("channels after delete = %d, want 1 (sibling kept)", n)
	}
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_stats_daily WHERE channel_id = $1`, aID); n != 0 {
		t.Fatalf("deleted channel stats rows = %d, want 0", n)
	}
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_stats_daily WHERE channel_id = $1`, bID); n != 1 {
...
func TestAdminRequeueDeadFreshTTL(t *testing.T) {
	...
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_deliveries WHERE id = $1 AND status = 'dead'`, deadID); n != 1 {
	...
	if err := db.QueryRowContext(ctx, `
		...
		WHERE d.id = $1`, deadID).Scan(&oldDeliveryExp, &oldMessageExp); err != nil {
		t.Fatal(err)
	}
	...
	if err := db.QueryRowContext(ctx, `
		SELECT d.expires_at, m.expires_at
		FROM novaque_deliveries d INNER JOIN novaque_messages m ON m.id = d.message_id
		WHERE d.id = $1`, deadID).Scan(&newDeliveryExp, &newMessageExp); err != nil {
		t.Fatal(err)
	}
	...
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_deliveries WHERE id = $1`, deadID); n != 1 {
...
func TestAdminListDeadPagination(t *testing.T) {
	...
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_deliveries WHERE channel_id = $1 AND status = 'dead'`, chID); n != total {
	...
	var maxID int64
	if err := db.QueryRowContext(ctx, `
		SELECT MAX(id) FROM novaque_deliveries WHERE channel_id = $1 AND status = 'dead'`, chID).Scan(&maxID); err != nil {
		t.Fatal(err)
	}
	if seen[0] != maxID {
...
func TestAdminDeleteDead(t *testing.T) {
	...
	var aliveID int64
	if err := db.QueryRowContext(ctx, `
		SELECT id FROM novaque_deliveries WHERE channel_id = $1 AND status = 'pending'`, chID).Scan(&aliveID); err != nil {
		t.Fatal(err)
	}
	...
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_deliveries WHERE id = $1`, deadIDs[1]); n != 0 {
	...
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_deliveries WHERE id = $1`, deadIDs[0]); n != 1 {
	...
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_deliveries WHERE id = $1`, aliveID); n != 1 {
...
func gotID(t *testing.T, ctx context.Context, db *sql.DB, chID int64) int64 {
	...
	var id int64
	if err := db.QueryRowContext(ctx, `
		SELECT id FROM novaque_deliveries WHERE channel_id = $1 AND status = 'dead' AND attempts = 2
