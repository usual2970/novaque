...
// upsert comfortably under parameter and statement-size limits.
...
// batched upserts. The day bucket comes from the DB clock
// ((NOW() AT TIME ZONE 'UTC')::date, never host time), so a flush spanning UTC
// midnight attributes its whole delta to the flush-time day. On error the
// not-yet-flushed deltas are merged back into the sink so a transient DB
// failure loses no counts (at-least-once).
func (s *Store) FlushStats(ctx context.Context) error {
	...
	for start := 0; start < len(order); start += statsFlushBatch {
		...
		args := make([]any, 0, (end-start)*(2+int(numStatKinds)))
		...
		if _, err := s.db.ExecContext(ctx, stmt, args...); err != nil {
			...
			return fmt.Errorf("postgres stats flush: %w", err)
		}
	}
	return nil
}
...
// come from statKind.column, so the enum and the SQL cannot drift. The UTC day
// expression is emitted per row (not a parameter); the $n numbering is
// continuous across all rows: each row carries topicID, channelID and the six
// counter values (2 + numStatKinds placeholders).
func statsFlushSQL(n int) string {
	...
	marksPerRow := 2 + int(numStatKinds)
	for i := 0; i < n; i++ {
		if i > 0 {
			b.WriteString(", ")
		}
		base := 1 + i*marksPerRow
		b.WriteString("((NOW() AT TIME ZONE 'UTC')::date")
		for j := 0; j < marksPerRow; j++ {
			b.WriteString(", $")
...
// one of the two package-known column names below, never caller input.
...
func (s *Store) ChannelCounters(ctx context.Context, channelID int64) (store.ChannelCounters, error) {
	if channelID <= 0 {
		return store.ChannelCounters{}, fmt.Errorf("postgres stats: invalid channel id %d", channelID)
	}
...
func (s *Store) TopicCounters(ctx context.Context, topicID int64) (store.ChannelCounters, error) {
	if topicID <= 0 {
		return store.ChannelCounters{}, fmt.Errorf("postgres stats: invalid topic id %d", topicID)
	}
...
const backlogSelectList = `
	  ...
	  COUNT(CASE WHEN status = $2 AND available_at <= ` + sqlNow + ` AND expires_at > ` + sqlNow + ` THEN 1 END),
...
// over the channel-status-available_at index prefix. Ready is the claimable
// slice of Pending and mirrors Claim's eligibility exactly (claim.go):
// available_at has passed the DB clock AND the delivery's TTL has not.
// Delayed publishes are excluded until due, and expired-but-not-yet-purged
// pending rows stay out of Ready (they can never be claimed; PurgeExpired
// reaps them on its tick) while Pending still counts them.
func (s *Store) ChannelBacklog(ctx context.Context, channelID int64) (store.ChannelBacklog, error) {
	if channelID <= 0 {
		return store.ChannelBacklog{}, fmt.Errorf("postgres stats: invalid channel id %d", channelID)
	}
...
// day boundary comes from the DB clock ((NOW() AT TIME ZONE 'UTC')::date),
// never host local time. PostgreSQL has no DELETE ... LIMIT, so each batch
// deletes by ctid over a subquery. The return value is the same total a single
// unbounded delete would report.
func (s *Store) PruneStats(ctx context.Context, retentionDays int) (int64, error) {
	if retentionDays <= 0 {
		return 0, fmt.Errorf("postgres stats: invalid retention days %d", retentionDays)
	}
	var total int64
	for {
		res, err := s.db.ExecContext(ctx, `
			...
			WHERE ctid IN (
				SELECT ctid FROM novaque_stats_daily
				WHERE day_utc < (NOW() AT TIME ZONE 'UTC')::date - make_interval(days => $1)
				LIMIT $2
