...
// Sequential stats integration tests (U1-U3), split from
// mysql_integration_test.go; concurrency coverage lives in
...
// connections carry a 1s lock-wait timeout, so the blocked tail upsert fails
// server-side (unambiguous rollback; a client ctx cancel would leave the
// at-least-once ambiguous window). The re-merge must restore exactly the
// tail's deltas (never the committed batch-1 ones), so the retry lands every
// delta exactly once.
func TestStatsFlushPartialBatchFailureReMerge(t *testing.T) {
	db := testmysql.Open(t)
	// The store gets its own handle whose connections all carry a 1s
	// innodb_lock_wait_timeout: the blocked tail upsert must fail SERVER-SIDE
	// (ER 1205). A client-side ctx cancel would be ambiguous — database/sql
	// returns while the killed statement may still complete once the lock
	// releases, and the retry would then double-count. That at-least-once
	// window is the accepted flush tradeoff, not what this test pins.
	flushDB, err := sql.Open("mysql", testmysql.DSN(t)+"&innodb_lock_wait_timeout=1")
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := db.ExecContext(ctx, `
		...
		VALUES (UTC_DATE(), ?, ?)`, topicID, lastID); err != nil {
		t.Fatal(err)
	}
	...
	if err := tx.QueryRowContext(ctx, `
		SELECT publish FROM novaque_stats_daily
		WHERE day_utc = UTC_DATE() AND topic_id = ? AND channel_id = ?
		FOR UPDATE`, topicID, lastID).Scan(&sink); err != nil {
		t.Fatal(err)
	}

	// Batch 1 (500 unlocked rows) commits; the tail blocks on the row lock and
	// fails server-side after the 1s lock-wait timeout — an unambiguous
	// statement rollback, so the retry cannot double-count it.
