...
// independent *sql.DB handles (two mysql.Store instances over the same shared
// MySQL) concurrently publish to the SAME channel and concurrently claim from
// it. Batched UPSERT flushes from the two connections must not lose updates,
// and after FlushStats on BOTH stores (each keeps its own in-process sink)
// the channel counters must equal the total successful mutations exactly:
// publish = successful publishes, claim = rows actually leased (sum of Claim
// result lengths, not attempts), ack = rows acked.
func TestStatsMultiWriterTwoStores(t *testing.T) {
	...
	side := func(s *mysql.Store, owner string) {
		for i := 0; i < perSide; i++ {
...
// 8 goroutines publish through ONE mysql.Store while a background goroutine
