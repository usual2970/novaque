...
// Admin-surface integration tests (U2): listings, batched backlogs, daily
...
// "UTC_DATE() - INTERVAL 2 DAY"), read from the DB clock so expectations
// never drift from the server's bucketing.
func dbDay(t *testing.T, ctx context.Context, db *sql.DB, expr string) time.Time {
	t.Helper()
	var s string
	if err := db.QueryRowContext(ctx, "SELECT DATE_FORMAT("+expr+", '%Y-%m-%d')").Scan(&s); err != nil {
...
func TestAdminDailyCounters(t *testing.T) {
	...
	seed("UTC_DATE()", chA, 2, 1, 0, 0, 0, 0)                   // today, channel A
	seed("UTC_DATE()", 0, 5, 0, 0, 0, 0, 0)                     // today, sentinel
	seed("UTC_DATE() - INTERVAL 2 DAY", chA, 0, 0, 4, 0, 0, 0)  // D-2, channel A
	seed("UTC_DATE() - INTERVAL 2 DAY", chB, 0, 0, 0, 0, 1, 0)  // D-2, channel B
	seed("UTC_DATE() - INTERVAL 10 DAY", chA, 0, 9, 0, 0, 0, 0) // outside every window
	...
	day2 := dbDay(t, ctx, db, "UTC_DATE() - INTERVAL 2 DAY")
	day0 := dbDay(t, ctx, db, "UTC_DATE()")
...
func TestAdminDeleteTopicCascade(t *testing.T) {
	...
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_messages WHERE topic_id = ?`, topicID); n != 3 {
		t.Fatalf("messages before delete = %d, want 3", n)
	}
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_deliveries WHERE channel_id IN (?, ?)`, aID, bID); n != 4 {
		t.Fatalf("deliveries before delete = %d, want 4", n)
	}
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_stats_daily WHERE topic_id = ?`, topicID); n != 3 {
	...
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_topics WHERE id = ?`, topicID); n != 0 {
		t.Fatalf("topics after delete = %d, want 0", n)
	}
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_channels WHERE topic_id = ?`, topicID); n != 0 {
		t.Fatalf("channels after delete = %d, want 0", n)
	}
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_messages WHERE topic_id = ?`, topicID); n != 0 {
		t.Fatalf("messages after delete = %d, want 0", n)
	}
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_deliveries WHERE channel_id IN (?, ?)`, aID, bID); n != 0 {
		t.Fatalf("deliveries after delete = %d, want 0 (cascaded)", n)
	}
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_stats_daily WHERE topic_id = ?`, topicID); n != 0 {
...
func TestAdminDeleteChannelCascade(t *testing.T) {
	...
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_deliveries WHERE channel_id = ?`, aID); n != 0 {
		t.Fatalf("deleted channel deliveries = %d, want 0", n)
	}
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_deliveries WHERE channel_id = ? AND status = 'in_flight'`, bID); n != 1 {
		t.Fatalf("sibling in_flight delivery = %d, want 1 (untouched)", n)
	}
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_messages WHERE id = ?`, msgID); n != 1 {
		t.Fatalf("shared message row = %d, want 1 (kept for the sibling)", n)
	}
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_channels WHERE topic_id = ?`, topicID); n != 1 {
		t.Fatalf("channels after delete = %d, want 1 (sibling kept)", n)
	}
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_stats_daily WHERE channel_id = ?`, aID); n != 0 {
		t.Fatalf("deleted channel stats rows = %d, want 0", n)
	}
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_stats_daily WHERE channel_id = ?`, bID); n != 1 {
...
func TestAdminRequeueDeadFreshTTL(t *testing.T) {
	...
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_deliveries WHERE id = ?`, deadID); n != 1 {
...
func TestAdminListDeadPagination(t *testing.T) {
	...
	if n := countRow(t, ctx, db, `SELECT COUNT(*) FROM novaque_deliveries WHERE channel_id = ? AND status = 'dead'`, chID); n != total {
