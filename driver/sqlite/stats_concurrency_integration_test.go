...
// TestStatsMultiWriterTwoStores covers U4 multi-writer correctness: two
// independent *sql.DB handles (two sqlite.Store instances over the same shared
// database file) concurrently publish to the SAME channel and concurrently
// claim from it. Batched upsert flushes from the two pools must not lose
// updates, and after FlushStats on BOTH stores (each keeps its own in-process
// sink) the channel counters must equal the total successful mutations
// exactly: publish = successful publishes, claim = rows actually leased (sum
// of Claim result lengths, not attempts), ack = rows acked.
func TestStatsMultiWriterTwoStores(t *testing.T) {
	path := testsqlite.Path(t)
	db1 := testsqlite.OpenPath(t, path, 10000)
	db2 := testsqlite.OpenPath(t, path, 10000)
	s1 := sqlite.New(db1)
	s2 := sqlite.New(db2)
	...
	side := func(s *sqlite.Store, owner string) {
		for i := 0; i < perSide; i++ {
...
// TestStatsSingleStoreConcurrentPublish covers U4 single-store concurrency:
// 8 goroutines publish through ONE sqlite.Store while a background goroutine
...
// TestStatsChannelIsolation covers U4 channel isolation: on one topic,
...
// channels' publish counters — one delivery per channel is the unit (KTD3) —
// so isolation is asserted on the claim/ack/requeue/dead/purge columns and
// on backlog. A zero-channel publish (before any channel exists) leaves both
...
// whichever store or process issued it — writes in one global order. SQLite
// permits only one writer at a time (WAL) and busy_timeout resolves the
// ordinary lock waits, so overlapping flushes cannot deadlock the way two
// InnoDB connections could (ER 1213); a failed flush round is still kept loud
// on purpose, and the totals assert no delta was lost or doubled either way.
func TestStatsConcurrentFlushLockOrder(t *testing.T) {
	...
	for r := 0; r < rounds; r++ {
		...
		for _, s := range []*sqlite.Store{s1, s2} {
			if _, err := s.Publish(ctx, topicID, []byte("m"), store.PublishOpts{TTL: time.Hour}); err != nil {
				t.Fatal(err)
			}
		}
